...
class ItemModelTest(TestCase):
	...
	def test_duplicate_items_are_invalid(self):
		...
		with self.assertRaises(ValidationError):
			...
			item.full_clean()
			#item.save() <- po make migrations zostanie nalozony constrain na bazie danych i save da nam IntegrityError z SQLite

	def test_can_save_item_to_diffrent_lists(self):
...
class ListModelTest(TestCase):
	...
	def test_list_ordering(self): #test added only to be sure, it depends from machine but
									  # flag `unique` set to true can break ordeing of items 
		list1 = List.objects.create()
	...
	def test_list_name_is_first_item_text(self):
		...
		self.assertEqual(list_.name, "item 1")
