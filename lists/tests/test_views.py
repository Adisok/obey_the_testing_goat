...
class ListViewTest(TestCase):
	...
	def test_duplicate_input_shows_error_on_page(self):
		...
		item1 = Item.objects.create(list=list1, text="dupl")
		response = self.client.post(
			f"/lists/{list1.id}/",
			data={"text": "dupl"}
			)
	...
	def test_for_invalid_errors_end_up_on_lists_page(self):
		response = self.post_invalid_input()
		self.assertEqual(response.status_code, 200)
		self.assertTemplateUsed(response, "list.html")
		self.assertContains(response, escape(EMPTY_ITEM_ERROR))
...
class NewListViewIntegratedTest(unittest.TestCase):
	...
	def setUp(self):
		...
		self.request.POST['text'] = 'new_list_item' #Build request by ourselves coz we use pure unittest
